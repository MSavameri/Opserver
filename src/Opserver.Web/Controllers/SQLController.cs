...
namespace Opserver.Controllers
{
    ...
    public partial class SQLController : StatusController<SQLModule>
    {
        ...
        [Route("sql/instance")]
        public async Task<ActionResult> Instance(string node)
        {
            var i = Module.GetInstance(node);

            if (i != null && i.CurrentCPUPercent >= Module.Settings.CpuMemoryUsage)
            {
            ...
            if (i != null && i.CurrentMemoryPercent >= Module.Settings.CpuMemoryUsage)
            {
                await _smsSender.Instance(Module.Settings.SmsSenderBaseUri,
                    Module.Settings.SmsSenderRequestUri,
            ...
            //var data = i.Databases.Data.SelectMany(x => x.Files);
            //foreach (var item in data)
            //{
            //    if (item.FileName == Module.Settings.TransactionLogFile && item.TotalSizeBytes >= Module.Settings.FreeDiskSpace)
            //    {
            //        await _smsSender.Instance(Module.Settings.SmsSenderBaseUri,
            //            Module.Settings.SmsSenderRequestUri,
            //            $"LOG {item.FileName} size:{item.TotalSizeBytes}",
            //            _httpClientName,
            //            _httpClient);
            //    }
            //}
